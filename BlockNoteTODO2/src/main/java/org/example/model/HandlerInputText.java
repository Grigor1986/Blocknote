...
public class HandlerInputText {
    ...
    public void handler() throws IOException, InterruptedException {
        ...
        switch (command) {
            case "C" -> {
                noteModelService.save(noteModelService.create());
            }
            case "U" -> {
                new OutputViewImpl().outputView(Messages.OUTPUTMESSAGEUPDATETASKS.getValue()
                        .formatted(new ViewMethodsImpl().outputAllTasks()));
                noteModelService.save(new NoteModelServiceImpl().update(Integer.parseInt(reader.readLine())));
            }
            case "V" -> {
            ...
            case "D" -> {
                new OutputViewImpl().outputView(Messages.OUTPUTMESSAGEDELETETASKS.getValue()
                        .formatted(new ViewMethodsImpl().outputAllTasks()));
                new ViewMethodsImpl().startProgram();
            }
            default -> {
                new OutputViewImpl().outputView("Введено неверное значение!\n");
                Thread.sleep(2000);
            }
        }
        new ViewMethodsImpl().startProgram();
